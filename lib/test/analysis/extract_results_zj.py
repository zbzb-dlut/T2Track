...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            if seq.dataset in ['trackingnet', 'got10k', 'lasot', 'lasot_extension_subset', 'otb', 'uavdt', 'visdrone',
                               'uavtrack112', 'dtb', 'uav123_10fps', 'uav123', 'nfs', 'tnl2k','uav123_l','webuav3m'] and '_onlynl' not in report_name:
