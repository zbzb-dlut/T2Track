...
from lib.test.analysis.extract_results_zj_auc_split import extract_results
import xlwt
        ...
        if seq_eval:
            ...
            for row in range(auc_seq.shape[0]):
                for col in range(auc_seq.shape[1]):
                    # auc_seq[row][col]<=25:     <=25
                    # auc_seq[row][col] <= 50 and auc_seq[row][col] > 25:   >25_and_<=50
                    # auc_seq[row][col] <= 75 and auc_seq[row][col] > 50:   >50_and_<=75
                    # auc_seq[row][col] > 75:    >75

                    # auc_seq[row][col]<=30:     <=30
                    # auc_seq[row][col] <= 70 and auc_seq[row][col] > 30:   >30_and_<=70
                    # auc_seq[row][col] > 70:    >70

                    # auc_seq[row][col]<=50:     <=50
                    # auc_seq[row][col] > 50:    >50

                    if auc_seq[row][col]<=50:
